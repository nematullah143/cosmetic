...
const ShoppingCarousel = () => {
  ...
  const nextSlide = () => {
    setIndex((prevIndex) => (prevIndex + 1) % (products.length - itemsPerView));
  };

  const prevSlide = () => {
    setIndex((prevIndex) => (prevIndex - 1 + (products.length - itemsPerView)) % (products.length - itemsPerView));
  };
  ...
  return (
    <div className="py-10 px-4 text-center w-full">
      <h2 className="text-2xl font-bold text-white mb-6 inline-block bg-black px-6 py-2 rounded-lg">
        Top Sellers
      </h2>
      <div className="relative flex items-center overflow-hidden w-full max-w-full mx-auto">
        <button 
          onClick={prevSlide} 
          className="absolute left-2 p-2 bg-gray-200 rounded-full shadow-md z-10"
        >
          <FaArrowLeft className="text-gray-700 text-lg" />
        </button>
        <div className="flex w-full overflow-hidden px-10 py-4 justify-center gap-4">
          {products.slice(index, index + itemsPerView).map((product) => (
            <div
              key={product.id}
              className="w-1/5 min-w-[200px] bg-white rounded-lg shadow-md flex-shrink-0 mx-2 transition-transform duration-500 ease-in-out hover:scale-105 cursor-pointer hover:-translate-y-2"
            >
              <img
        ...
        <button 
          onClick={nextSlide} 
          className="absolute right-2 p-2 bg-gray-200 rounded-full shadow-md z-10"
        >
